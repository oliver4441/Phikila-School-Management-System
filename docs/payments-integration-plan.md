...
- `payment_providers` — provider configuration per school.
...
- `payment_webhook_events` — provider event ID, payload hash, processing state, and timestamps for idempotency/audit.
- `payment_refunds` — refund requests and provider references.

Every school-owned record must carry `school_id`. Provider credentials remain server-side and must never be exposed through `VITE_*` variables.
...
3. Backend initializes the provider checkout/payment.
4. Customer completes authorization.
5. Provider sends a webhook.
6. Backend validates, verifies and processes the transaction idempotently.
7. Payment is allocated to the invoice.
8. Invoice balance and student account are updated transactionally.
9. Receipt is generated and exposed in the portal.

## Phase 3 — Direct M-PESA option

Add a `daraja` provider implementation after the first gateway path is stable.
...
Never ask users for an M-PESA PIN or secret credentials.

## Phase 4 — Finance integration

The Finance page should become the accounting view and registry for the payment layer.
...
- **direct payment recording by authorized finance staff**

A payment must not be marked as paid solely because the browser returned from checkout. The authoritative state comes from verified provider status/webhook processing or an authorized manual registry workflow.
...
No client-provided amount should be trusted. The backend should derive payable amounts from the current invoice state.
...
- Webhook endpoints: public HTTPS endpoint with provider verification.
- Never trust payment status from query parameters or browser state.
- Never expose database credentials, JWT secrets, or gateway secret keys to Vite.
...
4. Webhooks + verification + idempotency.
5. Invoice allocation and receipts.
6. Finance payment registry/manual entry.
7. Parent payment UI.
