...
studentsRoutes.get('/', async (c) => {
  ...
  if (ids.length) {
    const gs = await db.query(
      `${GUARDIAN_SELECT} where student_id = any($1) and student_id in (select id from students where school_id = ${sid}) order by id`,
      [ids],
    )
...
studentsRoutes.post('/', async (c) => {
  ...
  try {
    student = await insertRow(db, 'students', { school_id: sid, ...body, admission_number: admissionNumber })
  } catch (err) {
  ...
  const [full] = await db.query(`${STUDENT_SELECT} where id = $1 and school_id = ${sid}`, [student.id])
  const gs = await db.query(`${GUARDIAN_SELECT} where student_id = $1 and student_id in (select id from students where school_id = ${sid})`, [student.id])
...
studentsRoutes.get('/:studentId', async (c) => {
  ...
  const rows = await db.query(`${STUDENT_SELECT} where id = $1 and school_id = ${sid} limit 1`, [studentId])
  if (!rows[0]) return c.json({ detail: 'Student not found.' }, 404)
  const gs = await db.query(`${GUARDIAN_SELECT} where student_id = $1 and student_id in (select id from students where school_id = ${sid})`, [studentId])
...
studentsRoutes.patch('/:studentId', async (c) => {
  ...
  data.school_id = sid
  const updated = await updateStudent(db, c.req.param('studentId'), data, sid)
  if (!updated) return c.json({ detail: 'Student not found.' }, 404)
  const gs = await db.query(`${GUARDIAN_SELECT} where student_id = $1 and student_id in (select id from students where school_id = ${sid})`, [c.req.param('studentId')])
  return c.json({ ...updated, guardians: gs })
})

async function updateStudent(db: ReturnType<typeof createSql>, id: string, data: Record<string, unknown>, sid: number) {
  const keys = Object.keys(data)
  const sets = keys.map((k, i) => `"${k}" = $${i + 1}`).join(', ')
  const rows = await db.query(`update students set ${sets}, updated_at = now() where id = $${keys.length + 1} and school_id = ${sid} returning *`, [
    ...keys.map((k) => data[k]),
    id,
  ])
  return rows[0]
...
studentsRoutes.delete('/:studentId', async (c) => {
  ...
  const w = requireWrite(ten.ctx)
  if ('error' in w) return w.error
  const ok = await db.query(`select 1 from students where id = $1 and school_id = ${sid}`, [c.req.param('studentId')])
  if (!ok[0]) return c.json({ detail: 'Student not found.' }, 404)
  await deleteRowById(db, 'students', c.req.param('studentId'))
...
studentsRoutes.get('/:studentId/guardians', async (c) => {
  ...
  const sid = tenantSchoolId(ten.ctx)
  if (!sid) return c.json({ detail: 'Select a school first.' }, 400)
  const gs = await db.query(`${GUARDIAN_SELECT} where student_id = $1 and student_id in (select id from students where school_id = ${sid}) order by id`, [c.req.param('studentId')])
  return c.json(gs)
})
...
studentsRoutes.get('/:studentId/enrollment', async (c) => {
  ...
  const rows = await db.query(
    `select id, student_id, academic_year_id, term_id, class_id, level_id, stream_id, status, enrollment_date
     from enrollment_records where student_id = $1 and school_id = ${sid} order by id desc`,
...
studentsRoutes.post('/:studentId/enrollment', async (c) => {
  ...
  if ('error' in w) return w.error
  const body = await c.req.json().catch(() => ({}))
  const row = await insertRow(db, 'enrollment_records', { student_id: c.req.param('studentId'), school_id: sid, ...body })
  return c.json(row, 201)
})
...
studentsRoutes.get('/:studentId/documents', async (c) => {
  ...
  const rows = await db.query(
    `select id, student_id, document_type, title, description, file_url, file_size, mime_type, created_at
     from student_documents where student_id = $1 and student_id in (select id from students where school_id = ${sid}) order by id desc`,
    [c.req.param('studentId')],
  )
  return c.json(rows)
})
